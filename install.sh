...
usage() {
  arg0="$0"
  if [ "$0" = sh ]; then
    arg0="curl -fsSL https://github.com/edvillan15/code-server/install.sh | sh -s --"
  else
    not_curl_usage="The latest script is available at https://github.com/edvillan15/code-server/install.sh
"
  fi
...
More installation docs are at https://github.com/edvillan15/code-server/blob/master/doc/install.md
EOF
}

echo_latest_version() {
  # https://gist.github.com/lukechilds/a83e1d7127b78fef38c2914c4ececc3c#gistcomment-2758860
  version="$(curl -fsSLI -o /dev/null -w "%{url_effective}" https://github.com/edvillan15/code-server/releases/latest)"
  version="${version#https://github.com/edvillan15/code-server/releases/tag/v}"
  echo "$version"
}
...
install_deb() {
  ...
  fetch "https://github.com/edvillan15/code-server/releases/download/v$VERSION/code-server_${VERSION}_$ARCH.deb" \
...
install_rpm() {
  ...
  fetch "https://github.com/edvillan15/code-server/releases/download/v$VERSION/code-server-$VERSION-$ARCH.rpm" \
...
install_standalone() {
  ...
  fetch "https://github.com/edvillan15/code-server/releases/download/v$VERSION/code-server-$VERSION-$OS-$ARCH.tar.gz" \
